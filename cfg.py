...
def do_number_nodes(pydot_cfg):
    ...
    i = 0;
    for n in nodes:
        lines = n.get_label().split("\l")
        lines = [f"; n{i}"] + lines;
        n.set_label("\l".join(lines))
        i+=1;
    return pydot_cfg
...
@click.option('-o', '--output', required=True, type=str, help='Output PNG file name')
@click.option('--remove-assembly', is_flag=True, default=False, help="Remove assembly from basic blocks")
@click.option('--trim-addresses', is_flag=True, default=False, help="Remove addresses from assembly")
...
def do_cfg(file, output, symbol, spacing=1, trim_comments=False, remove_assembly=False, trim_addresses=False, number_nodes=False, jupyter=False, inst_counts=False, pretty_loops=False):
    r2 = open(f'{file}')
    listing = r2.cmd('aab; fs symbols; f')
    if not symbol:
        click.echo(listing)
        fcn_name = click.prompt('Enter a function symbol name from one of the above', type=str)
    else:
    ...
    if in_notebook() or jupyter:
        ...
        r = Image(output, embed=True)
        return r
    else:
        click.echo(f'CFG saved in {output}!')
